...
class collection:
    ...
    def fileName(self):
        self.collection_name = self.collection.split("/")

    # Read all the collections and its apis
    def readAllCollections(self):
        try:
...
def validateEnvironment(postman_environment):
    try:
        # Open the file with read mode
...
def validateCollection(postman_collection):
    try:
        # Open the file with read mode
